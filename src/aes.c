...
//#include "base64.h"
// Round Const
const unsigned char RC[10] = {0x01, 0x02, 0x04, 0x08, 0x10,
...
unsigned char mixcolumn3(unsigned char xch) { return xch ^ mixcolumn2(xch); }
...
unsigned char mixcolumn9(unsigned char xch) { return xch ^ mixcolumn8(xch); }
...
void key_expansion(const unsigned char* key, unsigned int* w) {
    ...
    for (i = 1; i <= 10; i++) {
        ...
        unsigned int temp;
        {
            unsigned char* pch;
            int j;
            temp = ((w[4 * i - 1] & 0xff000000) >> 24) |
                   ((w[4 * i - 1] << 8) & 0xffffff00);
            // printf("%x  ",temp);
...
void aes_enc(const char* plaintext, const char* key, char* ciphertext) {
    ...
    key_expansion((unsigned char*)key, w);
    // first addroundkey
    for (i = 0; i < 16; i++) {
    ...
    for (round = 1; round <= 10; round++) {
        ...
        if (round < 10) {
            ...
            tempresult[2] = mixtmp[0] ^ mixtmp[1] ^ mixcolumn2(mixtmp[2]) ^
                            mixcolumn3(mixtmp[3]);
            tempresult[3] = mixcolumn3(mixtmp[0]) ^ mixtmp[1] ^ mixtmp[2] ^
                            mixcolumn2(mixtmp[3]);
            ...
            tempresult[6] = mixtmp[4] ^ mixtmp[5] ^ mixcolumn2(mixtmp[6]) ^
                            mixcolumn3(mixtmp[7]);
            tempresult[7] = mixcolumn3(mixtmp[4]) ^ mixtmp[5] ^ mixtmp[6] ^
                            mixcolumn2(mixtmp[7]);
            ...
            tempresult[10] = mixtmp[8] ^ mixtmp[9] ^ mixcolumn2(mixtmp[10]) ^
                             mixcolumn3(mixtmp[11]);
            tempresult[11] = mixcolumn3(mixtmp[8]) ^ mixtmp[9] ^ mixtmp[10] ^
                             mixcolumn2(mixtmp[11]);
            ...
            tempresult[14] = mixtmp[12] ^ mixtmp[13] ^ mixcolumn2(mixtmp[14]) ^
                             mixcolumn3(mixtmp[15]);
            tempresult[15] = mixcolumn3(mixtmp[12]) ^ mixtmp[13] ^ mixtmp[14] ^
                             mixcolumn2(mixtmp[15]);
        }
        // add round key
        {
            unsigned char* pch;
...
void aes_dec(const char* ciphertext, const char* key, char* plaintext) {
    ...
    for (round = 1; round <= 10; round++) {
        ...
        // for (i = 0; i < 16; i++) printf("%x ", tempresult[i]);
    }
    memcpy(plaintext, tempresult, 16);
}

// ECB module design
// 最大输入的明文长度最大为4*1024字节,采用填充的方法即使是16的倍数，也要填充16个0x

int aes_encrypty(char* cipher, int* cipher_size, const char* plain,
                 int plain_size, const char* key, int key_size) {
    if (plain_size > Plain_Len_Max || plain_size < 0 || plain == NULL ||
        cipher == NULL || key == NULL) {
        return err_plain_size;
    }
    int block_num = plain_size / Block_Size;
    int remain = plain_size % Block_Size;
    int index = 0;
    char cipher_tmp[Block_Size] = {0};
    for (int i = 0; i < block_num; i++) {
        index = i * Block_Size;
        aes_enc(plain + index, key, cipher + index);
        // memcpy(cipher + index, cipher_tmp, Block_Size);
    }
    ...
    // memset(dst_base64, 0, Block_Size);
    // int dst_size = 0;
    // if (remain == 0) {
    //     index = Block_Size * block_num;
    //     memset(end_block, 0x10, Block_Size);
    //     aes_enc(end_block, key, cipher_tmp);
    //     memcpy(cipher + index, cipher_tmp, Block_Size);
    // }
    ...
    for (int i = 0; i < remain; i++) {
        end_block[i] = plain[index + i];
    }
    memset(end_block + remain, (char)(16 - remain), (16 - remain));
    aes_enc(end_block, key, cipher + index);
    // memcpy(cipher + index, cipher_tmp, Block_Size);
    *cipher_size = index + Block_Size;
    // if (remain <= 9) {
    //     base64_enc(dst_base64, &dst_size, plain + j, remain);
    //     dst_base64[Block_Size - 1] = dst_size;
    // }
    // for(int i=0;i<)
    //}
}
...
int aes_decrypty(char* plain, int* plain_size, const char* cipher,
    ...
    int block_num = cipher_size / Block_Size;
    int index = 0;
    char plain_tmp[Block_Size] = {0};
    for (int i = 0; i < block_num; i++) {
        index = i * Block_Size;
        aes_dec(cipher + index, key, plain + index);
        // memcpy(plain + index, plain_tmp,
        //        Block_Size);  //这个可以与上一句合并吗？
    }
    index = Block_Size * block_num;
    int remain = plain[index - 1];
    // int remain = 0;
    *plain_size = cipher_size - remain;
}
void iv_xor_input(char* iv, const char* input, int size) {
...
int aes_encrypty_cbc(char* cipher, int* cipher_size, const char* plain,
    ...
    if (plain_size > Plain_Len_Max || plain_size < 0 || IV_size < Block_Size ||
        plain == NULL || cipher == NULL || key == NULL || IV == NULL) {
        return err_plain_size;
    }
    char iv[Block_Size] = {0};
    memcpy(iv, IV, Block_Size);
    ...
    for (int i = 0; i < block_num; i++) {
        index = i * Block_Size;
        iv_xor_input(iv, plain + index, Block_Size);
        aes_enc(iv, key, cipher + index);
        memcpy(iv, cipher + index, Block_Size);
    }
    ...
    memset(end_block + remain, (char)(16 - remain), (16 - remain));
    iv_xor_input(iv, end_block, Block_Size);
    aes_enc(iv, key, cipher + index);
    // memcpy(cipher + index, cipher_tmp, Block_Size);
    *cipher_size = index + Block_Size;
}
int aes_decrypty_cbc(char* plain, int* plain_size, const char* cipher,
    ...
    if (plain == NULL || cipher == NULL || key == NULL || IV == NULL ||
        IV_size < Block_Size || cipher_size < Block_Size) {
        return err_plain_size;
    }
    char iv[Block_Size] = {0};
    memcpy(iv, IV, Block_Size);
    ...
    for (int i = 0; i < block_num; i++) {
        index = i * Block_Size;
        aes_dec(cipher + index, key, plain + index);
        iv_xor_input(plain + index, iv, Block_Size);
        memcpy(iv, cipher + index, Block_Size);
        // memcpy(plain + index, plain_tmp,
        //        Block_Size);  //这个可以与上一句合并吗？
    }
