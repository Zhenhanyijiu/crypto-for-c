...
// aes encrypt
void aes_enc(const char* plaintext, const unsigned char* key,
             char* ciphertext) {
  unsigned int w[44];
  int i, j, round;
  ...
  for (round = 1; round <= 10; round++) {
    ...
    if (round < 10) {
      ...
      tempresult[0] =
          mixcolumn2(mixtmp[0]) ^ mixcolumn3(mixtmp[1]) ^ mixtmp[2] ^ mixtmp[3];
      tempresult[1] =
          mixtmp[0] ^ mixcolumn2(mixtmp[1]) ^ mixcolumn3(mixtmp[2]) ^ mixtmp[3];
      tempresult[2] =
          mixtmp[0] ^ mixtmp[1] ^ mixcolumn2(mixtmp[2]) ^ mixcolumn3(mixtmp[3]);
      tempresult[3] =
          mixcolumn3(mixtmp[0]) ^ mixtmp[1] ^ mixtmp[2] ^ mixcolumn2(mixtmp[3]);

      tempresult[4] =
          mixcolumn2(mixtmp[4]) ^ mixcolumn3(mixtmp[5]) ^ mixtmp[6] ^ mixtmp[7];
      tempresult[5] =
          mixtmp[4] ^ mixcolumn2(mixtmp[5]) ^ mixcolumn3(mixtmp[6]) ^ mixtmp[7];
      tempresult[6] =
          mixtmp[4] ^ mixtmp[5] ^ mixcolumn2(mixtmp[6]) ^ mixcolumn3(mixtmp[7]);
      tempresult[7] =
          mixcolumn3(mixtmp[4]) ^ mixtmp[5] ^ mixtmp[6] ^ mixcolumn2(mixtmp[7]);
...
// aes decrypt
void aes_dec(const char* ciphertext, const unsigned char* key,
             char* plaintext) {
  unsigned int w[44];
  int i, j, round;
  unsigned char tempresult[16];
